...
def particion_entr_prueba(X,y,test=0.20):
    ...
    random.shuffle(X_train),random.shuffle(y_train),random.shuffle(X_test),random.shuffle(y_test)
...
def CONSTRUYE_ARBOL(X,y,min_ejemplos_nodo_interior,max_prof,prop_umbral,prof=0):
    ...
    if (prof >= max_prof) or (num_ejemplos < min_ejemplos_nodo_interior) or (len(np.unique(y)) < 2):
        valor_hoja,distribucion = calcular_valor_hoja(y)
        return Nodo(clase = valor_hoja,distr = distribucion)
    
    mejor_division = get_mejor_division(X,y,num_ejemplos,num_columnas,prop_umbral)
    
    if len(mejor_division) != 0:
        if mejor_division["ganancia_info"] > 0:
            x_izq = np.array(mejor_division["dataset_izq"][:,:-1])
            x_der = np.array(mejor_division["dataset_der"][:,:-1])
            if(type(y[0]) == type(int)):
                y_izq = mejor_division["dataset_izq"][:,-1:].flatten().astype(int).tolist()
                y_der = mejor_division["dataset_der"][:,-1:].flatten().astype(int).tolist()
            else:
                y_izq = mejor_division["dataset_izq"][:,-1:].flatten().astype(str).tolist()
                y_der = mejor_division["dataset_der"][:,-1:].flatten().astype(str).tolist()
            
            

            
            arbol_izq = CONSTRUYE_ARBOL(x_izq,y_izq,min_ejemplos_nodo_interior= min_ejemplos_nodo_interior,max_prof=max_prof,prop_umbral=prop_umbral,prof=prof+1)
            arbol_der = CONSTRUYE_ARBOL(x_der,y_der,min_ejemplos_nodo_interior=min_ejemplos_nodo_interior,max_prof=max_prof,prop_umbral=prop_umbral,prof=prof+1)
            
            return Nodo(atributo = mejor_division["indice_atr"],umbral = mejor_division["umbral"],
                        izq = arbol_izq , der = arbol_der)
    
def get_mejor_division(X,y,num_ejemplos,num_columnas,prop_umbral):
    mejor_division = {}
    ganancia_max_info = -float("inf")
    for indice_atr in range(num_columnas):
        valores_atr = X[:,indice_atr]
        posibles_umbrales = get_posibles_umbrales(valores_atr,y,prop_umbral)
        dataset = X.tolist()
        y_p = y
        
        for i in range(0,len(X)-1):
            dataset[i].append(y_p[i])
        dataset.pop(-1)
        for umbral in posibles_umbrales:
            dataset_izq,dataset_der = division(dataset,indice_atr,umbral)
            
            # print(len(dataset_izq),len(dataset_der))
            if(len(dataset_izq)>0 and len(dataset_der)>0):
                y_izq, y_der = dataset_izq[:,-1], dataset_der[:,-1]
                
                ganancia_info = ganancia_informacion(y,y_izq,y_der)
                
                # print(ganancia_info,ganancia_max_info)
                if ganancia_info > ganancia_max_info:
                    mejor_division["indice_atr"] = indice_atr
                    mejor_division["umbral"] = umbral
                    mejor_division["dataset_izq"] = dataset_izq
                    mejor_division["dataset_der"] = dataset_der
                    mejor_division["ganancia_info"] = ganancia_info
                    ganancia_max_info = ganancia_info 
    
    return mejor_division

def get_posibles_umbrales(valores_atributo,y,prop_umbral):
    res = []
    datos = sorted(zip(valores_atributo,y), key=lambda e:e[0])
    
    for i in range(0,len(datos)-2):
        
        if (datos[i][0] != datos[i+1][0]) and (datos[i][1] != datos[i+1][1]):
            res.append((float(datos[i][0])+float(datos[i+1][0]))/2)
...
def division(dataset,indice_atr,umbral):
    dataset_izq,dataset_der = [],[]
    
    for i in dataset:
        
        if(float(i[indice_atr])<=umbral):  
            dataset_izq.append(i)
        else:
            dataset_der.append(i)
    return np.array(dataset_izq),np.array(dataset_der)

def ganancia_informacion(padre, hijo_izq, hijo_der):       
        peso_izq = len(hijo_izq) / len(padre)
        peso_der = len(hijo_der) / len(padre)
        ganancia = entropia(padre) - (peso_izq*entropia(hijo_izq) + peso_der*entropia(hijo_der))
        return ganancia
...
def entropia(y):
    ...
    return -np.sum(probabilidades * np.log2(probabilidades + np.finfo(float).eps))  # Añadimos un pequeño épsilon para evitar el logaritmo de cero


def calcular_valor_hoja(y):
    
    valores , conteos = np.unique(y,return_counts=True)
    atr_max = np.argmax(conteos)
    
    distr = dict()
    for i,j in zip(valores,conteos):
        distr[i] = j
    
    return valores[atr_max],distr
